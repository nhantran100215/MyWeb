                        ...
                        device.forEach((eachdevice,deviceIndex)=>{
                            if(eachdevice.deviceInfo[us.info][us.ID_board]==ID_boardtp){
                                switch(mode){
                                    ...
                                    case 3:{
                                        eachdevice.relay[relayIndex].mode2={mode:2,setting:{}};
                                        eachdevice.relay[relayIndex].mode2.setting=obj[us.sync];
                                        let numberOfTimer=obj[us.sync][us.alarm].length;
                                        device[deviceIndex].relay[relayIndex].timertag=[];
                                        for (let j=0;j<numberOfTimer;j++){
                                            createDisplayTimer(false,ID_boardtp,deviceIndex,relayIndex,obj[us.sync][us.alarm][j][us.alarm_time],obj[us.sync][us.alarm][j][us.state],obj[us.sync][us.alarm][j][us.repeat],obj[us.sync][us.alarm][j][us.weekday],j);
                                        }
                                    }break;
                                    case 4:{
                                        if(obj[us.sync]==1){
...
function createDevicetabs(devicename,counter,ID_board,frametabs=1){
    ...
    device[counter].devicename.timerSelect.addEventListener('change',function(event){
        let ID_boardtp=document.getElementsByClassName('timer__selected__Switch')[0].value;
        device.forEach((eachdevice,i)=>{
            eachdevice.relay.forEach(eachrelay=>{
                eachrelay.timertag.forEach(eachtimertag=>{
                    eachtimertag.style.display='none';
                });
            });
            if(eachdevice.deviceInfo[us.info][us.ID_board]==ID_boardtp){
                eachdevice.relay[this.value].timertag.forEach(eachtimertag=>{
                    eachtimertag.style.display='block';
                })
            }
        })
        if(this.value!="") document.getElementsByClassName('timer__create__btn')[0].style.display='block';
    })
...
timer__create.getElementsByClassName('timer__areaCreate')[0].addEventListener('click',function(event){
...
timer__create.getElementsByClassName('timer__create__btn')[0].addEventListener('click',function timerCreateBtn(){
    ...
    timer__create.getElementsByClassName('alarm__mode__state')[0].style.backgroundColor='blanchedalmond';
    timer__create.getElementsByClassName('alarm__mode__repeat')[0].style.backgroundColor='blanchedalmond';
    timer__create.getElementsByClassName('alarm__mode__weekday')[0].style.display='none';


})
////create button >save
timer__create.getElementsByClassName('alarm__btn__save')[0].addEventListener('click',function(){
    let alarm_time=timer__create.getElementsByClassName('alarm__time__text')[0].value;
    let state,repeat,weekday="";
    if(timer__create.getElementsByClassName('alarm__mode__state')[0].style.backgroundColor=='blanchedalmond') state=1;
    else state=0;
    if(timer__create.getElementsByClassName('alarm__mode__repeat')[0].style.backgroundColor=='blanchedalmond') repeat=0;
    else if(timer__create.getElementsByClassName('alarm__mode__repeat')[1].style.backgroundColor=='blanchedalmond') repeat=1;
    else {
        repeat=2;
        let alarm__mode__weekday__input=timer__create.getElementsByClassName('alarm__mode__weekday__input');
        for (let i=0;i<alarm__mode__weekday__input.length;i++){
           if(alarm__mode__weekday__input[i].checked) weekday=weekday+"1";
           else weekday=weekday+"0";
        }
    }
    let ID_board=document.getElementsByClassName('timer__selected__Switch')[0].value;
    console.log(weekday);
    // let Dcounter,Rcounter,timerIndex;
    console.log(ID_board);
    // console.log(relayIndex);
    device.forEach((eachdevice,i)=>{
        if(eachdevice.deviceInfo[us.info][us.ID_board]==ID_board){
            let relayIndex=eachdevice.devicename.timerSelect.value;
            console.log(relayIndex);
            let timerIndex=eachdevice.relay[relayIndex].timertag.length
            createDisplayTimer(true,ID_board,i,relayIndex,alarm_time,state,repeat,weekday,timerIndex);
            let alarm=eachdevice.relay[relayIndex].mode2[us.setting][us.alarm];
            alarm[alarm.length]={
                [us.alarm_time]:alarm_time,
                [us.state]:state,
                [us.repeat]:repeat,
                [us.weekday]:weekday,
            ...
            // console.log(alarm);
            // console.log(eachdevice.relay[relayIndex].mode2[us.setting][us.alarm]);
            let string=JSON.stringify(eachdevice.relay[relayIndex].mode2);
            // console.log(string);
            SendMessage(eachdevice.relay[relayIndex].topic.setting,string);
        }
    })
    timer__create.getElementsByClassName('timer__areaCreate')[0].style.display='none';
})
///////////timer switch select 
...
function createDisplayTimer(createTimer,ID_board,Dcounter,Rcounter,time,state,repeat,weekday,timerIndex){
    let alarm={};
    ...
    if (createTimer==false)alarm.style.display='none';
    ...
    if(repeat==2) alarm__mode__weekday.style.display='flex';
    ...
    for(let i=0;i<7;i++){
        ...
        alarm__mode__weekday__input[i].id=(('alarm__mode__weekday__input'+ID_board)+timerIndex)+(i)+Rcounter;
        ...
        alarm__mode__weekday__label[i].htmlFor=alarm__mode__weekday__input[i].id;
        alarm__mode__weekday__label[i].textContent=arrWeekday[i];
        if(repeat=2)
        {
            if(weekdaytp==1) {
                alarm__mode__weekday__label[i].classList.add('borderAnimation');
                alarm__mode__weekday__input[i].checked=true;
            }
        }
    ...
    let lengthtp=device[Dcounter].relay[Rcounter].timertag[timerIndex].getElementsByClassName('alarm__mode__weekday__input').length;
    console.log('lengthtp:');
    console.log(lengthtp);
    for(let j=0;j<lengthtp;j++){
        device[Dcounter].relay[Rcounter].timertag[timerIndex].getElementsByClassName('alarm__mode__weekday__input')[j].addEventListener("change",function(){
            let alarm__mode__weekday__label=device[Dcounter].relay[Rcounter].timertag[timerIndex].getElementsByClassName('alarm__mode__weekday__label')[j];
            if(this.checked){
                console.log('checked');
                device[Dcounter].relay[Rcounter].timertag[timerIndex].getElementsByClassName('alarm__mode__weekday__label')[j].classList.add("borderAnimation");
                console.log(alarm__mode__weekday__label);
            }else{
                console.log('unchecked');
                device[Dcounter].relay[Rcounter].timertag[timerIndex].getElementsByClassName('alarm__mode__weekday__label')[j].classList.remove("borderAnimation");
                console.log(alarm__mode__weekday__label);
            }
            console.log(device[Dcounter].relay[Rcounter].timertag[timerIndex].getElementsByClassName('alarm__mode__weekday__input')[j]);
        })
    }
...
document.addEventListener("click", closeCurrentAlarm);
    ...
    if(determineTimer!=-1){
        ...
        device.forEach((eachdevice,i)=>{
            if(eachdevice.deviceInfo[us.info][us.ID_board]==ID_board){
                let relayIndex=eachdevice.devicename.timerSelect.value;
                eachdevice.relay[relayIndex].timertag.forEach((timertagloop,j)=>{
                    console.log(timertagloop.getElementsByClassName('alarm__time__text')[0].value)
                    // console.log(eachdevice.relay[relayIndex].mode2);

                    eachdevice.relay[relayIndex].mode2[us.setting][us.alarm][j][us.alarm_time]=timertagloop.getElementsByClassName('alarm__time__text')[0].value;

                    if(timertagloop.getElementsByClassName('alarm__mode__state')[0].style.backgroundColor=="blanchedalmond"){
                        eachdevice.relay[relayIndex].mode2[us.setting][us.alarm][j][us.state]=1;
                    }else{
                        eachdevice.relay[relayIndex].mode2[us.setting][us.alarm][j][us.state]=0;
                    }

                    if(timertagloop.getElementsByClassName('alarm__mode__repeat')[0].style.backgroundColor=="blanchedalmond"){
                        eachdevice.relay[relayIndex].mode2[us.setting][us.alarm][j][us.repeat]=0;
                    }else if(timertagloop.getElementsByClassName('alarm__mode__repeat')[1].style.backgroundColor=="blanchedalmond"){
                        eachdevice.relay[relayIndex].mode2[us.setting][us.alarm][j][us.repeat]=1;
                    }else{
                        eachdevice.relay[relayIndex].mode2[us.setting][us.alarm][j][us.repeat]=2;
                        let alarm__mode__weekday__input=timertagloop.getElementsByClassName('alarm__mode__weekday__input'),weekdaytp='';
                        for(let m=0;m<alarm__mode__weekday__input.length;m++){
                            if(alarm__mode__weekday__input[m].checked){
                                weekdaytp=weekdaytp+1;
                            }else{
                                weekdaytp=weekdaytp+0;
                            }
                        }

                        eachdevice.relay[relayIndex].mode2[us.setting][us.alarm][j][us.weekday]=weekdaytp;
                    }
